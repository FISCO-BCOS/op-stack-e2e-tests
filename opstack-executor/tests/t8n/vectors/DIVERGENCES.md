...
<!-- P3-1b devnet batch (historical): the deposit-creation arms of the old 884-block
     devnet chain vector. SAME base-layer class as the rows above (FINDING-create-output,
     vendored evmone Host::create() carries no output_data; FISCO receipt `output` is a
     non-consensus TARS extension field, receiptsRoot unaffected). The upgrade-activation
     blocks each redeploy the fork's predeploy implementations via creation deposits, so
     this class is inherent to any devnet corpus that spans activations.
     P5 (2026-09-16): the registered devnet snapshot moved to the auto-fit tail window
     (95MiB register ceiling; state dumps are ~9.4MB each against this genesis, so a
     window spanning activation blocks does not fit). The registered vector
     devnet_1875-2274_c38db356 (post-campaign jovian-segment tail, op-geth pin
     e8800cffe, genesis self-check 2340 accounts + storage join verified) replays with
     ZERO create-output divergences -- the activation redeploys are absent from the
     window. The 10 rows pinned to the replaced devnet_884_007399eb are therefore
     removed rather than re-pinned; the class itself stays covered by the 6
     *_contract_create rows and the 8 ladder rows above. When a future window again
     spans activation blocks, the replay's DIVERGE output re-derives this batch's
     exact rows (opdevnet.sh snapshot's "next" hint prints the ritual). -->
