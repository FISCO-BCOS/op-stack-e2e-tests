...
#   - vectors/manifest.txt        注册表（幂等 append；WI-E13 起 §4c item 3/12 随
#                                 loader 扩表一并入册，无豁免项）
...
# static：§4c 12 项，isthmus base 出 1..10+12（item 11 恒用 jovian base）；jovian base
# 只补 fork 无关的 blob 面 item 3/12（WI-E13）→ isthmus 11 文件 + jovian 3 文件。
# FISCO 的 GoldenSample loader 已支持把 _op_payload 的 expectedBlobVersionedHashes /
# executionRequests 透传进 engine_newPayloadV4 的 params[1]/params[3]；isthmus 锚定的
# item 1..10 不在 jovian base 重出（锚定随 fork 变化：9B/17B extraData、pre-Jovian
# blobGasUsed 零 vs Jovian DA 等值门），单 isthmus 变体即覆盖。
"$OPGETH/opt8n-ref" --mode=static --base isthmus_transfer_basic --out-dir "$T8N_DIR/vectors" --op-geth-commit "$PIN"
"$OPGETH/opt8n-ref" --mode=static --base jovian_transfer_basic --out-dir "$T8N_DIR/vectors" --op-geth-commit "$PIN"
...
# append-if-absent：已注册的向量（非注释非空行）不再追加。WI-E13 起 §4c item 3/12
# 随 FISCO loader 扩表一并入册，static 面不再有任何豁免项。
append_if_absent() {
...
# observer 惯例），历史注释不改写。
blob_static_vectors=(invalid_isthmus_transfer_basic_static_3.json invalid_isthmus_transfer_basic_static_12.json
                     invalid_jovian_transfer_basic_static_3.json invalid_jovian_transfer_basic_static_12.json)
is_blob_static() {
  local base="$1"
  for b in "${blob_static_vectors[@]}"; do
    [ "$base" = "$b" ] && return 0
  done
  return 1
}
...
sorted_blob=()
while IFS= read -r line; do sorted_blob+=("$line"); done < <(printf '%s\n' "${blob_static_vectors[@]}" | sort)
append_if_absent "$manifest" "WI-E13 blob/requests static vectors: §4c items 3/12 (bothForks) + the jovian static base, registered once the GoldenSample loader passes engine_newPayloadV4 params[1]/params[3] through" "${sorted_blob[@]}"
sorted_obs=()
